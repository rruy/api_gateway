...
class Rule
  ...
  def match
    @options[:match].instance_of?(String) ? Regexp.new("#{@options[:match]}$") : @options[:match]
  end
  ...
  def get_frequency
    case @options[:metric]
    when :rpd
      ((86_400 / @options[:limit]).zero? ? 1 : 86_400 / @options[:limit])
    when :rph
      ((3600 / @options[:limit]).zero? ? 1 : 3600 / @options[:limit])
    when :rpm
      ((60 / @options[:limit]).zero? ? 1 : 60 / @options[:limit])
    end
  end
